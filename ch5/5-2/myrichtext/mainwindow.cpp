    ...
    ui(new Ui::MainWindow)
{
    ...
    //5.2.2 文本块
    QAction *action_textFrame = new QAction(QString::fromLocal8Bit("框架"), this);
    ...
    QAction *action_textBlock = new QAction(QString::fromLocal8Bit("文本块"), this);
    connect(action_textBlock, &QAction::triggered, this, &MainWindow::showTextBlock);
    ui->mainToolBar->addAction(action_textBlock);

    QAction *action_font = new QAction(QString::fromLocal8Bit("字体"), this);
    ...
    btn->setText(QString::fromLocal8Bit("查找下一个"));
    ...
    //5.2.5 语法高亮与HTML
//    highlighter = new MySyntaxHighlighter(ui->textEdit->document());   //语法高亮照着写报错，很难受

    ui->textEdit->append(QString::fromLocal8Bit("<h1><font color=red>使用HTML</font></h1>"));
}
...
void MainWindow::showTextBlock()          // 遍历文本块
{
    ...
    {
        qDebug() << QString::fromLocal8Bit("文本块%1，文本块首行行号为:%2，长度为:%3,内容为：")
...
void MainWindow::setTextFont(bool checked)         // 设置字体格式
{
    if(checked){                                   // 如果处于选中状态
        ...
        // 使用宋体，12号，加粗，倾斜
        charFormat.setFont(QFont(QString::fromLocal8Bit("微软雅黑"), 12, QFont::Bold, true));
        charFormat.setFontUnderline(true);         // 使用下划线
        cursor.setCharFormat(charFormat);          // 使用字符格式
        cursor.insertText(QString::fromLocal8Bit("测试字体"));         // 插入文本
    }
...
void MainWindow::findNext()     // 查找下一个
{
    ...
    if(isfind){                // 如果查找成功，输出字符串所在行和列的编号
        qDebug() << QString::fromLocal8Bit("行号:%1 列号:%2")
